...
// byte-history map.
//
// Reads `data/asset-bytes-registry.jsonl` (the
...
export function loadExclusionKeys(exclusionDoc) {
  ...
  for (const [idx, ex] of exclusionDoc.exclusions.entries()) {
    if (!ex.card_id || !ex.byte_sha256) {
      // Fail-closed on malformed entries (PR #79). The exclusions
      // file is operator-authored build-time input — a typo like
      // `byte_sha265` would otherwise silently let a misroute back
      // into byte-history.json. Throwing here names the offending
      // entry directly. Identifiers only — a valid byte_sha256
      // would make the line ~150 chars.
      throw new Error(
    ...
    // Catch typo'd audit fields before they accumulate (PR #79). A
...
export function buildByteHistory(rows, exclusionKeys = new Set()) {
  ...
  for (const row of rows) {
    ...
    if (!cardId) continue;
    // A row with missing byte_sha256 (PR #79)
  ...
  for (const cardId of sortedCardIds) {
    ...
    //
    // Comparator MUST return 0 on equal keys (PR #71): the
...
function main() {
  ...
  if (existsSync(EXCLUSIONS)) {
    ...
    // Prior shape warned-and-continued
