// Byte-size + percentage-delta formatters used across the
...
// add to one without the other (PR #79 — prior
 ...
 * producing broken or surprising links at runtime. PR #79 —
 ...
 * names by the /altered/ pages. PR #79:
...
export function categoryClass(category: string | null | undefined): string {
  ...
  // the CSS class binding and the data-category filter slot.
  // PR #79: prior fallback was `altered-cat-unknown`
 ...
 * ``data-filter="unverified"`` value). PR #79: the prior raw
 * interpolation let operator typos
