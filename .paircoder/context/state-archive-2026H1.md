# State Archive — 2026 H1 (through 2026-07-11)

> Verbatim moment-in-time session/sprint logs and the prior canonical status
> block, moved out of `state.md` on 2026-07-13 during the doc-hygiene pass.
> **Not edited** — these are historical records; some engine/stat/command
> references (Surya, `--engine auto`, alex-zhang42 augmentation, page counts)
> reflect the state at time of writing and are superseded. For current status
> see `state.md`.
...
Upstream dropped Release 02 (64 new cards dated 5/22/26) overnight as expected. Our 30-min cron poll reported "unchanged" all night because it was pointed at `uap-release001.csv` — a URL that upstream froze at Release-01-only content when they consolidated everything under a new mutating canonical CSV at `uap-data.csv`.

### Root cause

`src/pursue_index/config/settings.py::Settings.csv_url` default was set on 2026-05-12 to `…/uap-release001.csv` on the theory that upstream was moving to per-release filenames (`release001` → `release002` → …). The comment even called the pattern out. Reality on 2026-05-22: upstream took the opposite path — they kept a single mutating canonical file (`uap-data.csv`) with every release stacked into it (Release 01 = 158 rows from 5/8/26, Release 02 = 64 rows from 5/22/26, total 222 rows / 297 KB). The hash-stable comparison against the frozen `release001.csv` byte stream therefore stayed pinned at the old sha forever.

No poll-failure alert fired because the fetch itself was succeeding cleanly — the wrong URL was returning a stable 200 with stable bytes.

### What landed (branch `claude/second-tranche-poll-miss-GeofS`)

| File | Change |
|---|---|
| `src/pursue_index/config/settings.py` | `csv_url` default → `…/uap-data.csv`; comment rewritten to record the consolidation + the two-step URL history. |
| `data/last-known-csv-sha.txt` | `f75e2f7de0ff…` (release001-only) → `f9ad3b8366e2…` (release001+02 consolidated). |
| `data/raw/csv/f9ad3b8366e2….csv` | 297,933 bytes — the canonical Release 02 byte stream, content-addressed per the existing archive convention. |
| `web/scripts/build_csv_archive.mjs` | `source_url` default in the emitted `index.json` repointed. |
| `web/public/data/csv/{f9ad…csv,index.json}` | Regenerated by running the build script; index now includes 7 archived CSVs newest-first with the updated source URL. |
| `web/src/pages/methodology.astro` | /methodology Source provenance copy + manifest example updated to reflect the `uap-data.csv` URL + the two-step rotation history. |

41 tests in `test_poll_pursue*.py`, `test_csv_fetcher.py`, `test_manifest.py` pass. `bpsai-pair arch check` clean on the modified settings file.

### What's still pending (operator-attended)

The capture step (poll + byte archive) is now done. Heavy ingest is still operator-attended by design (GPU provisioning, cost, content review) — none of these are kicked off here:

1. Merge `claude/second-tranche-poll-miss-GeofS` → `main` so the scheduled poll cron uses the new URL.
2. Run `pursue scrape run` → produces a refreshed `data/manifests/latest.json` containing all 222 cards.
3. Run `pursue download run --manifest …` → fetches the 64 new PDFs/images into NAS / R2.
4. Run `pursue ocr run --manifest … --engine auto` → OCR + LLM cleanup for the new cards.
5. Run `pursue embed run --manifest …` → Voyage-3 embeddings for the new pages.
6. Web rebuild → /search, /atlas, /gallery, /removed all pick up the new tranche.

Tranche-detected GitHub issue did **not** auto-fire (we updated `last-known-csv-sha.txt` directly to capture the bytes); the operator should open one manually if the tracking-issue lane is part of the heavy-ingest checklist, or trigger `workflow_dispatch` on `.github/workflows/poll-pursue.yml` after merge to exercise the issue + R2-archive lanes against the now-current sha.

### Follow-up to consider

The `uap-data.csv` URL is the third the canonical CSV has lived at in two weeks (`uap-csv.csv` → `uap-release001.csv` → `uap-data.csv`). Each rotation costs us at least one cron cycle and one careful sha comparison to detect. Worth considering: have the poll also fetch `war.gov/UFO/` (the parent HTML page) and scrape the `csvUrl = "…"` JS literal as a sanity check that our pinned URL matches the one upstream is actually serving traffic to. Out of scope for this hotfix; logged here.
